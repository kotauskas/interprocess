...
///
/// The difference between this trait and [`TryInto`]`<`[`LocalSocketName`]`>` is that the latter does not constrain the
...
/// As mentioned in the [`LocalSocketName` documentation][`LocalSocketName`], there are two types of which local socket
/// names can be: filesystem paths and namespaced names. Those are isolated from each other – there's no portable way to
/// represent one using another, though certain OSes might provide ways to do so – Windows does, for example. To be able
/// to represent both in a platform-independent fashion, a special syntax was implemented in implementations of this
/// trait on types from the standard library: "@ syntax".
///
/// The feature, in its core, is extremely simple: if the first character in a string is the @ character, the value of
/// the string is interpreted and stored as a namespaced name (otherwise, it's treated as a filesystem path); the @
...
/// - If the string is a [`OsStr`]/[`OsString`], it can be cheaply converted to a [`Path`]/[`PathBuf`], which do not
///   support the @ syntax
...
///
/// None of the above conversions perform memory allocations – the only expensive one is [`CStr`]/[`CString`] which
/// performs a check for valid UTF-8.
///
...
/// support namespaced socket names, the result is always a file-type name; on platforms that do, prefixing the name
/// with the `@` character will trim it away and yield a namespaced name instead. See the trait-level documentation for
/// more.
...
/// names, the result is always a file-type name; on platforms that do, prefixing the name with the `@` character will
...
/// don't support namespaced socket names, the result is always a file-type name; on platforms that do, prefixing the
/// name with the `@` character will trim it away and yield a namespaced name instead. See the trait-level documentation
/// for more.
...
/// Converts an owned [`String`] to an owned [`LocalSocketName`]. On platforms which don't support namespaced socket
/// names, the result is always a file-type name; on platforms that do, prefixing the name with the `@` character will
/// trim it away and yield a namespaced name instead. See the trait-level documentation for more.
...
/// Converts a borrowed [`CStr`] to a borrowed [`LocalSocketName`] with the same lifetime. **UTF-8 is assumed and the
/// nul terminator is preserved during conversion**. On platforms which don't support namespaced socket names, the
/// result is always a file-type name; on platforms that do, prefixing the name with the `@` character will trim it away
...
/// preserved during conversion**. On platforms which don't support namespaced socket names, the result is always a
/// file-type name; on platforms that do, prefixing the name with the `@` character will trim it away and yield a
