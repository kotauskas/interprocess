...
//! is closed, its corresponding end of the pipe is no longer accessible. Unnamed pipes generally
//! only make sense to use when communicating with child processes.
//!
...
//!
//! Note [the standard library's support for creating unnamed pipes for `stdin`, `stdout` and `stderr`](std::process::Stdio),
//! which should be used in simple cases instead of this module. Making use of `std`-provided pipe
//! creation is recommended if the program of the child process can be modified to communicate
//! with its parent via standard I/O streams, since unnamed pipes feature comparatively little
//! overhead, have consistent behavior across platforms, and play nicely with command-line
//! utilities.
//!
//! # Examples
//! See [`pipe()`].
