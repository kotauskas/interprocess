...
///
/// This is a sort of const-generic incarnation of the [`PipeMode`] enumeration that allows `PipeStream` to be
/// consolidated into a single struct with generic parameters that decide which traits are implemented on it.
///
/// Some examples of how different `PipeStream` signatures would look:
/// - **`PipeStream<Bytes, Bytes>`** (or, thanks to default generic arguments, simply `PipeStream`) is a duplex stream
///   that receives and sends bytes.
