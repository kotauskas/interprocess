...
///     - It must not return any different 𝑛 until [`.set_len()`] is called with some value 𝑚 as its argument, after
///       which [`.valid_len()`] must return 𝑚.
///     - The first 𝑛 bytes of the slice returned by [`.as_bytes()`]/[`.as_bytes_mut()`] must be valid in the ancillary
///       buffer validity sense as described in the [module-level docs](super).
/// - As long as the safety contract of [`.set_len()`] is upheld, [`.as_bytes()`]`.len()` must never go below
///   [`.valid_len()`].
...
///     - [`.valid_len()`]
///
/// [`.as_bytes()`]: CmsgMut::as_bytes
/// [`.as_bytes_mut()`]: CmsgMut::as_bytes_mut
/// [`.valid_len()`]: CmsgMut::valid_len
/// [`.set_len()`]: CmsgMut::set_len
/// [`.reserve()`]: CmsgMut::reserve
pub unsafe trait CmsgMut {
    ...
    ///
    /// # Safety
    /// The valid part of the slice (as designated by the return value of [`.valid_len()`](CmsgMut::valid_len)) may not
    /// be modified in ways that compromise its validity. Ideally, [`.split_at_init()`](CmsgMutExt::split_at_init)
    ...
    /// - `new_len` must not exceed the capacity (given by [`.capacity()`](CmsgMutExt::capacity)).
    ...
    /// Returns whether the control message list was truncated during reception.
    fn is_truncated(&self) -> bool;
    /// Sets the flag returned by [`.is_truncated()`](CmsgMut::is_truncated).
    ...
    /// [`.as_bytes()`](CmsgMut::as_bytes)/[`.as_bytes_mut()`](CmsgMut::as_bytes_mut).
    fn reserve(&mut self, additional: usize) -> ReserveResult {
        let _ = additional;
        Err(ReserveError::Unsupported)
    }
    /// Like [`.reserve()`](CmsgMut::reserve), but hints the underlying buffer data structure not to purposely
    /// overallocate. The memory allocator may still choose to overallocate.
