...
pub trait Collector {
    ...
    /// and the contents of the `msghdr`'s `msg_flags` field which it will be performed with.
    fn pre_op_collect(&mut self, socket: BorrowedFd<'_>, msghdr_flags: c_int) {}
    /// Same as `pre_op_collect`, but called right after the system call.
    fn post_op_collect(&mut self, socket: BorrowedFd<'_>, msghdr_flags: c_int) {}
}
impl<T: Collector> Collector for &mut T {
    #[inline]
    fn pre_op_collect(&mut self, socket: BorrowedFd<'_>, msghdr_flags: c_int) {
        (*self).pre_op_collect(socket, msghdr_flags);
    }
    #[inline]
    fn post_op_collect(&mut self, socket: BorrowedFd<'_>, msghdr_flags: c_int) {
...
impl<T: Collector> Collector for Box<T> {
    #[inline]
    fn pre_op_collect(&mut self, socket: BorrowedFd<'_>, msghdr_flags: c_int) {
        self.as_mut().pre_op_collect(socket, msghdr_flags);
    }
    #[inline]
    fn post_op_collect(&mut self, socket: BorrowedFd<'_>, msghdr_flags: c_int) {
...
impl<F1: FnMut(BorrowedFd<'_>, c_int), F2: FnMut(BorrowedFd<'_>, c_int)> FnCollector<F1, F2> {
...
impl<F1: FnMut(BorrowedFd<'_>, c_int)> FnCollector<F1, fn(BorrowedFd<'_>, c_int)> {
...
impl<F2: FnMut(BorrowedFd<'_>, c_int)> FnCollector<fn(BorrowedFd<'_>, c_int), F2> {
...
impl<F1: FnMut(BorrowedFd<'_>, c_int), F2: FnMut(BorrowedFd<'_>, c_int)> Collector for FnCollector<F1, F2> {
    fn pre_op_collect(&mut self, socket: BorrowedFd<'_>, msghdr_flags: c_int) {
        self.0(socket, msghdr_flags)
    }
    fn post_op_collect(&mut self, socket: BorrowedFd<'_>, msghdr_flags: c_int) {
    ...
    for<'a> <&'a mut C as IntoIterator>::Item: Collector,
{
    fn pre_op_collect(&mut self, socket: BorrowedFd<'_>, msghdr_flags: c_int) {
        for mut c in &mut self.0 {
            c.pre_op_collect(socket, msghdr_flags);
        }
    }
    fn post_op_collect(&mut self, socket: BorrowedFd<'_>, msghdr_flags: c_int) {
        for mut c in &mut self.0 {
